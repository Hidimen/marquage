...
## Why Marquage
There already has many markup language, such as `JSON`, `YAML` and `TOML`. `Marquage` is not designed to replace them, but **provide a totally new way to store data, manage your config files and transfer statistics.**

## How to use
```rust
use marquage_lib::{ from_str, Parse, Generate };
